...
export type AppSchema = {
  [Path: string]: {
    [Method: string]: {
      input: unknown; // Body | Query
      output: unknown; // Response
    };
  };
};

export type Context<S extends RouteSchema, Env = Record<string, unknown>> = Env & {
  event: H3Event;
...
export type MiddlewareHandler<Env = Record<string, unknown>> = (
  ctx: Context<RouteSchema, Env>,
  next: Next
) => Promise<void> | void;
